...
def add_segmentation_outputs_yolov9s_preserve_ir(input_model_path, output_model_path):
    """
    Add recommended segmentation layer outputs to YOLOv9-S model
    while preserving the original ir_version
    Based on recommendations from segmentation_analysis_report_yolov9s.md
    """
    ...
    # Recommended layers from segmentation_analysis_report_yolov9s.md
    # Priority order based on the report recommendations
    recommended_layers = [
        # 最優先推奨
        "/model.11/Concat_output_0",   # 40x40, 448ch, score: 0.3063 - 最適なバランス
        
        # 高解像度融合層
        "/model.14/Concat_output_0",   # 80x80, 320ch, score: 0.2750 - 詳細な境界線
        
        # 深層特徴融合層
        "/model.8/Concat_output_0",    # 20x20, 512ch, score: 0.3219 - 豊富な意味特徴
        
        # 早期融合層
        "/model.2/Concat_output_0",    # 160x160, 128ch, score: 0.2500 - 高解像度輪郭
        
        # 超高解像度Swish層（オプション - 超高精度が必要な場合）
        "/model.0/act/Mul_output_0"    # 320x320, 32ch, score: 0.3687 - 最高解像度
    ]
    
    # Layer descriptions for documentation
    layer_descriptions = {
        "/model.11/Concat_output_0": "Primary feature (16x downsample, best balance)",
        "/model.14/Concat_output_0": "High-resolution feature (8x downsample)",
        "/model.8/Concat_output_0": "Deep semantic feature (32x downsample, max channels)",
        "/model.2/Concat_output_0": "Early fusion feature (4x downsample, fine details)",
...
if __name__ == "__main__":
    input_model = "yolov9_s_wholebody25_Nx3x640x640.onnx"
    output_model = "yolov9_s_wholebody25_Nx3x640x640_featext.onnx"
    
    print("YOLOv9-S Segmentation Feature Extraction Model Generator")
    print("=" * 60)
    print("This script adds recommended segmentation layer outputs")
    print("based on segmentation_analysis_report_yolov9s.md")
    print("=" * 60)
    
    added = add_segmentation_outputs_yolov9s_preserve_ir(input_model, output_model)
    ...
    if added:
        ...
        print("Recommended usage based on requirements:")
        print("  - Balance: model.11 + model.14 (best accuracy/speed)")
        print("  - Speed: model.11 only (single layer)")
        print("  - Accuracy: model.2 + model.14 + model.11 (multi-scale)")
        print("  - Research: All layers (maximum features)")
    else:
